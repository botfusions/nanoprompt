...
async function exportFlatSheet() {
    console.log("=== Exporting Flat Sheet (One Prompt Per Line) ===\n");
    ...
    // Convert to CSV with Flattening (Removing Newlines)
    const headers = ['ID', 'Title', 'Prompt', 'Use Case', 'Visual Style', 'Score', 'Reason'];
    const csvRows = [headers.join(',')];

    for (const p of filteredPrompts) {
        // FLATTENING LOGIC: Replace newlines with spaces
        const cleanTitle = (p.title || '').replace(/[\r\n]+/g, ' ').replace(/"/g, '""');
        const cleanPrompt = (p.prompt || '').replace(/[\r\n]+/g, ' ').replace(/"/g, '""');
    ...
    const fileName = `top_prompts_flat_${new Date().toISOString().split('T')[0]}.csv`;
    const outputPath = path.resolve(process.cwd(), fileName);

    fs.writeFileSync(outputPath, csvContent, 'utf8');

    console.log(`\n✅ EXPORT SUCCESS! (Flattened)`);
    console.log(`----------------------------------------`);
    console.log(`📉 Initial Count:    ${prompts.length}`);
    console.log(`🗑️  Removed (Lang):   ${removedCounts.language}`);
    console.log(`🗑️  Removed (Info):   ${removedCounts.infographic}`);
